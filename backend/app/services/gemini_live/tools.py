...
def live_system_instruction(language: str) -> dict:
    ...
    if language == "en":
        text = (
            ...
            "- NEVER mention tool names (ADJUST_VALUE, SELECT_COLOR, etc.) to users. These are internal implementation details.\n"
            ...
            "- After executing a tool call, respond naturally without mentioning the tool used (e.g., \"Made it brighter!\" not \"Used ADJUST_VALUE to increase brightness\").\n"
        )
    else:
        text = (
            "# Role\n"
            ...
            "\n"
            "## コミュニケーションスタイル（重要）\n"
            "- ユーザーに対してツール名（ADJUST_VALUE、SELECT_COLORなど）を絶対に言及しないでください。これらは内部実装の詳細です。\n"
            ...
            "- tool callを実行した後は、使用したツールに言及せず自然に応答してください（例：「明るくしました！」であって「ADJUST_VALUEツールで明度を上げました」ではない）。\n"
        )
