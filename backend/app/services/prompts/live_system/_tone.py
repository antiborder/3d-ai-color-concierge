...
- **tool call 後のフォローアップ**: action（tool callの実行結果を1文で報告）+ optional suggestion（次のアクション提案、必要に応じて1文）の2文以内に収めること。次のアクション提案のバリエーションは「次のアクション提案のバリエーション」セクションを参照。\
"""
...
- **Follow-up after tool calls**: keep it to 2 sentences max — one reporting the action result + optionally one varied suggestion for what to try next. For what to suggest, refer to the "Vary your follow-up suggestions" section.\
"""
