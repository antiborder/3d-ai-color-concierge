...
from app.api.schemas.colors import CategoryResponse, ColorItem, ColorSearchResponse
...
async def search_colors(
    query: str | None = Query(None, description="色名やHEXコードで検索"),
    tags: str | None = Query(None, description="カンマ区切りでタグを指定 (CSS,MATERIAL,JAPANESE)"),
    ...
    try:
        ...
        results = color_service.search(query=query, tags=tag_list, limit=limit, offset=offset)
...
async def get_color_by_hex(hex_code: str):
    ...
    try:
        ...
        if not color:
            raise HTTPException(status_code=404, detail=f"Color not found for hex code: {hex_code}")
        return color
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting color by hex: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error getting color: {str(e)}")
