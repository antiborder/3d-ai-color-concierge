...
const GradientBar = ({ colorA, colorB, shape, onColorSelect }: GradientBarProps) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const [r, g, b] = interpolateRgb(colorB, colorA, shape, t);
    onColorSelect(r, g, b);
  };

  const gradient = buildGradientStops(colorB, colorA, shape);

  return (
    <Wrapper>
      <Label>
        <ColorDot style={{ background: `rgb(${colorB.r},${colorB.g},${colorB.b})` }} />
        <Bar ref={barRef} style={{ background: gradient }} onClick={handleClick} />
        <ColorDot style={{ background: `rgb(${colorA.r},${colorA.g},${colorA.b})` }} />
      </Label>
    </Wrapper>
  );
};
